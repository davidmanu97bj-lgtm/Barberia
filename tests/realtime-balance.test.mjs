...
const engine=createRequire(import.meta.url)('../functions/period-settlement.js');
...
for (const [kind,preview,ids,delta] of [
  ...
  ['uber','renderUberAccountPreview',['uberPrincipalPreview','uberCashboxPreview'],55000]
]) test(`${kind}: el impacto no se vuelve a sumar al recibir el alta, pero sí refresca otros cambios antes de guardar`, () => {
  let balance=0;
  const nodes=new Map(),$=id=>{if(!nodes.has(id))nodes.set(id,{value:'100000',dataset:{},classList:{toggle(){},add(){},remove(){}},textContent:'',innerHTML:''});return nodes.get(id);};
  $('chargeMode').value='cash';
  const ctx=vm.createContext({$,activeSubmissionLocks:new Set(),submissionPreviewBalances:new Map(),submissionPreviewModels:new Map(),ExploraPeriodSettlement:engine,
    settlementModel:()=>({...engine.calculate(),balance}),scheduleDashboardRender(){},parseMoneyInput:Number,parseUberAmount:Number,
  ...
  load(ctx,'acquireSubmissionLock','releaseSubmissionLock','captureSubmissionBalance','previewSettlementBalance','previewDefinition',preview);
...
for (const [kind,operation,delta] of [['charge','cash',55000],['expense','expense',-50000]])
 test(`${kind}: quitar el bloque visual no elimina el snapshot interno ni duplica el impacto`,()=>{
  let balance=0;
  const ctx=vm.createContext({activeSubmissionLocks:new Set(),submissionPreviewBalances:new Map(),submissionPreviewModels:new Map(),ExploraPeriodSettlement:engine,
    settlementModel:()=>({...engine.calculate(),balance}),scheduleDashboardRender(){}});
  load(ctx,'acquireSubmissionLock','releaseSubmissionLock','captureSubmissionBalance','previewSettlementBalance','previewDefinition');
  assert.equal(ctx.previewSettlementBalance(kind),0);
  balance=200;assert.equal(ctx.previewSettlementBalance(kind),200);
  assert.equal(ctx.acquireSubmissionLock(kind),true);assert.equal(ctx.acquireSubmissionLock(kind),false);
  balance=300;ctx.captureSubmissionBalance(kind);
  assert.equal(ctx.previewDefinition(operation,100000).delta,delta);
  balance+=delta;assert.equal(ctx.previewSettlementBalance(kind),300);
  ctx.releaseSubmissionLock(kind);assert.equal(ctx.previewSettlementBalance(kind),balance);
 });
